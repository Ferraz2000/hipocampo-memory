...
Wired as Claude Code `SessionEnd`, Codex `Stop`, or Gemini `SessionEnd` — all
pass a `transcript_path` on stdin. `SessionEnd` is the right home: it fires once
at true session end (unlike `Stop`, which fires after every assistant turn and
would re-run this consolidation sweep mid-session). Stdin JSON (fields vary by
agent, all optional):
{"transcript_path", "session_id"/"sessionId", "stop_hook_active", "reason"}.

Behavior:
- Skip on stop_hook_active (Codex `Stop` re-entrancy; absent on SessionEnd) or
  reason=="clear" (a deliberate `/clear` — Claude SessionEnd / Gemini — isn't a
  session worth sweeping), or if /capture was invoked this session.
...
def _render_sweep(date, short, session_id, sweep_type, triggers_hit, urls,
    ...
    lines = ["---",
             ...
             "> Generated by the capture-sweep session-end hook. The session ended without",
...
def main(argv=None):
    try:
        ...
        return 0
    # Codex `Stop` re-entrancy guard (absent on Claude/Gemini SessionEnd); skip
    # `reason == "clear"` — a deliberate `/clear` (Claude SessionEnd / Gemini) is
    # not a genuine session end. Both fields absent on agents that don't send them.
